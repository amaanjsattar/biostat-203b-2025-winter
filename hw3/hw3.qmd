...
ggplot(chartevents_viz, aes(x = charttime, y = valuenum, color = abbreviation,
  ...
  scale_x_datetime(
    breaks = seq(
      floor_date(min(chartevents_viz$charttime, na.rm = TRUE), unit = "6 hours"), 
      ceiling_date(max(chartevents_viz$charttime, na.rm = TRUE), unit = "6 hours"), 
      by = "6 hours"
    ),
    labels = function(x) {
      # Only show every other tick label
      labels <- format(x, "%b %d %H:%M")
      labels[seq(1, length(labels), 2)] <- ""
      return(labels)
    }
  ) +  
