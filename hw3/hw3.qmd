...
top_diagnoses <- diagnoses_icd %>%
  ...
  arrange(seq_num) %>%  # ✅ Sort by sequence number (lower = more primary)
  slice_head(n = 3) %>%
  pull(long_title)
...
ggplot(data = timeline_events_viz) +
  geom_segment(
    ...
    aes(
      ...
      color = careunit,
      linewidth = is_icu_ccu == "ICU/CCU"  # ✅ ICU/CCU gets a larger linewidth
    )
  ) +
  
  geom_point(
    data = timeline_events_viz %>% filter(event_type == "Lab"),
    aes(x = admittime, y = event_type),
    shape = 3, size = 3, color = "black"
  ) +

  geom_point(
    data = timeline_events_viz %>% filter(event_type == "Procedure"),
    aes(x = admittime, y = event_type, shape = procedure_desc),
    size = 3, color = "black"
  ) +
