...
We will attach the abbreviations for vital measurements by performing a join on `itemid` between `chartevents_viz` and `items`. We will convert timezone to UTC to ensure proper formatting for the plot. We now have all of our information aggregated into a singular dataframe, and we can begin plotting.
```{r}
...
**Solution**: We will first import the data, and then utilize `mutate` to specify our data types. Once again, we want to ensure that timezone is `UTC` and that all data types are properly stored. We can use the `str` command to verify this periodically,
```{r}
...
**Solution**:
First, we will see how many unique values of `subject_id` there are. We observe that there are 65,366 unique instances of subject id. To check if a particular value for `subject_id` can correspond to more than one icu stay, characterized by 'stay_id'. We will do this by creating a summary table 
```{r}
...
**Solution**: We will import the data and then ensure that all data types are properly formatted.
```{r}
...
ggplot(admissions_tble,
  ...
  scale_x_continuous(breaks = seq(0,
                                  ceiling(max(admissions_tble$length_of_stay,
                                              na.rm = TRUE)),
                                  by = 25)) +
...
**Solution**: We will first import the data. Next, we will make sure that all data types are properly formatted. This was done in the console using `str`.
```{r}
...
```{r}
# Define vital item IDs for HW3 Q6 (use 220180 for diastolic BP)
vitals_items <- c(220045, 220179, 220180, 223761, 220210)

# Manually define the final column names (based on items$label)
final_names <- c(
  "220045" = "Heart Rate",
  "220179" = "SBP",      # Systolic BP
  "220180" = "DBP",      # Diastolic BP (for HW3 Q6)
  "223761" = "Temperature (F)",
  "220210" = "Respiratory Rate"
)

# Process chartevents:
# 1. Load only relevant columns and filter for the desired itemids.
# 2. Join with icustays_tble to get ICU info (subject_id, stay_id, intime).
# 3. Keep only measurements with storetime >= intime.
# 4. For each ICU stay and vital (itemid), select the earliest storetime (averaging if duplicates exist).
# 5. Pivot wider so that each ICU stay is one row with columns for each vital.
chartevents_tble <- open_dataset("chartevents_pq", format = "parquet") %>%
  ...
  # Keep only measurements recorded at or after ICU admission
  filter(storetime >= intime) %>%
  
  # For each ICU stay and vital, keep the earliest measurement (if there are ties, average the values)
  group_by(subject_id, stay_id, itemid) %>%
  slice_min(storetime, n = 1, with_ties = TRUE) %>%
  summarise(avg_valuenum = mean(valuenum, na.rm = TRUE), .groups = "drop") %>%
  
  # Pivot so that each ICU stay is one row, columns for each vital item
  pivot_wider(
    names_from = itemid,
...
# Rename the lab columns using the final_names mapping.
# The pivoted columns are named like "lab_220045", etc.
for(old in names(final_names)) {
  old_name <- paste0("lab_", old)
  new_name <- final_names[old]
  if(old_name %in% colnames(chartevents_tble)) {
    colnames(chartevents_tble)[colnames(chartevents_tble) == old_name] <- new_name
  }
}

# Optionally, filter out rows where all vital measurements are NA.
chartevents_tble <- chartevents_tble %>%
  filter(if_any(-c(subject_id, stay_id), ~ !is.na(.)))

# Print the final table and number of rows
