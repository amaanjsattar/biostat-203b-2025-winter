...
**Solution**:
```{r}
# Define the relevant item IDs for vitals and our desired final labels.
...
chartevents_tble <- open_dataset("chartevents_pq", format = "parquet") %>%
  ...
  filter(storetime >= intime) %>%
  # For each ICU stay and vital, select the first measurement (using storetime)
  ...
  pivot_wider(names_from = itemid, values_from = avg_valuenum, names_prefix = "lab_") %>%
  collect() %>%
  arrange(subject_id, stay_id) %>%
  relocate(subject_id, stay_id, sort(names(.)))

# Rename pivoted columns using the mapping in final_names.
...
# Round the numeric columns (all except subject_id and stay_id) to 2 decimal places.
chartevents_tble <- chartevents_tble %>%
  mutate(across(-c(subject_id, stay_id), ~ round(.x, 2)))

# Optionally, remove rows that have all NA for the lab measurements.
chartevents_tble <- chartevents_tble %>%
  filter(if_any(-c(subject_id, stay_id), ~ !is.na(.)))

# Display the final result and row count
