...
Exact execution times for each function are displayed above. We observe that utilizing the `base r` function `read.csv` was the most time-intensive, taking approximately 9.6 seconds to complete. Using `read.csv` from the `tidyverse` was notably faster, with an execution time of approximately 4.7 seconds. The fastest function won by a considerable margin, being the `fread` function from the `data.table` package. This function took less than one second to execute. 
