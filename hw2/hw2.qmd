...
```
We observe that after specifying the column types ourselves and reading in the data, the process is both faster and more memory-efficient. The runtime on the user end is just over 1 second (1.085 sec, to be exact), compared to > 4.5 seconds (4.736 sec, to be exact) when we didn't specify column types. Similarly, we observe that memory usage is 63.47 MB, compared to 200.1 MB  when types were not pre-specified. Ultimately, by avoiding unnecessary character-types, this user specification is able to greatly enhance the speed and storage efficiency of our ingesting process.
...
# Ingestion step
system.time({
  labevents.parquet <- arrow::open_dataset('part-0.parquet', 
                      format = 'parquet') %>%
    select(subject_id, itemid, charttime, valuenum) %>%
    filter(itemid %in% c(
      50912, 
