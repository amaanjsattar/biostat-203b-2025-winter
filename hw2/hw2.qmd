...
We observe that there are 32,679,896 lines, excluding our header. Now, we will display the first ten lines of our new file. 
```{bash}
...
# filter itemid
system.time({
  labevents_filtered <- open_dataset(
    'labevents.csv', 
    format = 'csv') %>%
...
```
We observe that the operation took just shy of one minute to complete, which is much faster than the operations we conducted that were reliant upon local memory. Now, we will display the number of rows, as well as the first ten rows of the filtered dataset.
